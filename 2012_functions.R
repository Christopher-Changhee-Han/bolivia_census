#### Helper Functions to read and process 2012 census data ####

#### FUNCTION 1: clean_data_2012 ####
# helper function to do initial data cleaning for get_2012 function
clean_data_2012 <- function(data){
...
get_2012 <- function(data){
  
  # check to see if you have clean data or original data, and convert to clean
  if (names(data)[1] != "1") df <- clean_data_2012(data) # rename columns, get rid of initial NA's
  else df <- data
  ...
  for (i in 1:(length(index) - 1)){
    ...
    noaplica <- grep("Ignorado :", pull(df[index[i]:index[i+1],],1)) # get index of Ignorado
